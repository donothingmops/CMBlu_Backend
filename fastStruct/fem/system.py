...
from fastStruct.basic import FEMException, arg_to_list
from fastStruct.fem import plotter, system_components
from fastStruct.fem.elements import Element
from fastStruct.fem.postprocess import SystemLevel as post_sl
from fastStruct.fem.util.load import LoadCase
from fastStruct.sectionbase import properties
from fastStruct.vertex import Vertex, vertex_range

if TYPE_CHECKING:
    from matplotlib.figure import Figure

    from fastStruct.fem.node import Node
    from fastStruct.types import (
        AxisNumber,
        Dimension,
        LoadDirection,
        MpType,
...
class SystemElements:
    ...
    def show_structure(
        ...
        # Return plotting values if requested; useful for custom plotting (e.g., with Plotly)
        if values_only:
            return self.plot_values.structure()

        # Generate the structure plot with specified parameters
        fig = self.plotter.plot_structure(figsize=figsize, verbosity=verbosity, 
                                        show=show, supports=supports, 
                                        scale=scale, offset=offset, 
                                        annotations=annotations)

        # For web integration, 'show' is False; figure can be saved to file or encoded for embedding
        if not show:
            # Example: fig.savefig("structure_plot.png")
            # Or return the figure for further processing (e.g., encoding to base64 for web embedding)
            return fig
