...
from fastStruct.basic import integrate_array
from fastStruct.fem.node import Node

if TYPE_CHECKING:
    from fastStruct.fem.elements import Element
...
class SystemLevel:
    ...
    def report_max_bending_moments(self):
        print("Maximum Bending Moments for each Element:")
        max_moment_array = {}

        for element_id, element in self.system.element_map.items():
            max_moment = getattr(element, 'max_bending_moment', 'Not Calculated')
            print(f"Element {element_id}: {max_moment:.3f} kNm")

            max_moment_array[element_id] = f"{max_moment:.3f}"
        print("Max Bending Moment(kNm)= ", max_moment_array)


    def report_max_shear_forces(self):
        print("Maximum Shear Forces for each Element:")
        max_shear_force_array = {}

        for element_id, element in self.system.element_map.items():
            max_shear = getattr(element, 'max_shear_force', 'Not Calculated')
            if isinstance(max_shear, (int, float)):
                print(f"Element {element_id}: {max_shear:.3f} kN")
                max_shear_force_array[element_id] = f"{max_shear:.3f}"
            else:
                print(f"Element {element_id}: {max_shear}")
                max_shear_force_array[element_id] = max_shear

        print("Max Shear Force(kN)= ", max_shear_force_array)


    def report_max_displacements(self):
        print("Maximum Displacements for each Element:")
        max_displacement_array = {}

        for element_id, element in self.system.element_map.items():
            max_disp = getattr(element, 'max_displacement', 'Not Calculated')
            if isinstance(max_disp, (int, float)):
                print(f"Element {element_id}: {max_disp:.3f} m")
                max_displacement_array[element_id] = f"{max_disp:.3f}"
            else:
                print(f"Element {element_id}: {max_disp}")
                max_displacement_array[element_id] = max_disp

        print("Max Displacement(m)= ", max_displacement_array)


    def report_max_axial_forces(self):
        print("Maximum Axial Forces for each Element:")
        max_axial_force_array = {}

        for element_id, element in self.system.element_map.items():
            max_axial = getattr(element, 'max_axial_force', 'Not Calculated')
            if isinstance(max_axial, (int, float)):
                print(f"Element {element_id}: {max_axial:.3f} kN")
                max_axial_force_array[element_id] = f"{max_axial:.3f}"
            else:
                print(f"Element {element_id}: {max_axial}")
                max_axial_force_array[element_id] = max_axial

        print("Max Axial Force(kN)= ", max_axial_force_array)
...
class ElementLevel:
    ...
    def determine_bending_moment(element: "Element", con: int) -> None:
        ...
        # After calculating m_val, store the max absolute moment in the element
        element.max_bending_moment = np.max(np.abs(element.bending_moment))
